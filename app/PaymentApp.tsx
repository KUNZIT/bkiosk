...
export default function PaymentApp() {
    ...
    const sendCommand = useCallback(
        async (command: string) => {
            ...
            try {
                ...
                // If writing fails, likely disconnected.
    ...
    const disconnectFromArduino = useCallback(async () => {
        if (port) {
            try {
                if (reader) {
                    // Important: Cancel the reader before closing the port to release the lock
                    await reader.cancel();
    ...
    useEffect(() => {
        ...
        const readSerialData = async () => {
            ...
            try {
                ...
                // If read loop breaks due to error, the port is likely closed/crashed
        ...
        return () => {
            loop = false; // Signal the loop to stop
            // Attempt to cancel the read promise if the effect cleans up
